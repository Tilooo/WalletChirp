...
onMounted(async () => {
  ...
  // tom set the initial names for default codes
...
<template>
  <div class="bg-slate-900 text-slate-200 min-h-screen p-4 sm:p-8 flex flex-col items-center">
    ...
    <main class="w-full max-w-7xl grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div class="flex flex-col gap-8">
        <div class="bg-slate-800 p-6 rounded-lg shadow-lg">
          ...
          <div class="flex gap-4">
            <input id="country1" v-model="countryName1" @keyup.enter="fetchData(1)" list="country-list" placeholder="Type a country name..." class="w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-lime-500" />
        ...
        <div class="bg-slate-800 p-6 rounded-lg shadow-lg min-h-[220px] flex items-center justify-center">
          ...
          <div v-else-if="countryData1 && countryData1.country" class="w-full">
            <h2 class="text-2xl font-bold mb-4 border-b border-slate-700 pb-2">{{ countryData1.country.name }}</h2>
            <ul class="space-y-3">
              <!-- POPULATION -->
              <li v-if="countryData1.population" class="flex justify-between items-center">
                <span class="font-semibold text-sm">Population ({{ countryData1.population.year }})</span>
                <span class="text-lime-300 font-mono text-lg">{{ formatNumber(countryData1.population.value) }}</span>
              </li>
      ...
      <div class="flex flex-col gap-8">
        <div class="bg-slate-800 p-6 rounded-lg shadow-lg">
          ...
          <div class="flex gap-4">
            <input id="country2" v-model="countryName2" @keyup.enter="fetchData(2)" list="country-list" placeholder="Type a country name..." class="w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-lime-500" />
        ...
        <div class="bg-slate-800 p-6 rounded-lg shadow-lg min-h-[220px] flex items-center justify-center">
          ...
          <div v-else-if="countryData2 && countryData2.country" class="w-full">
            <h2 class="text-2xl font-bold mb-4 border-b border-slate-700 pb-2">{{ countryData2.country.name }}</h2>
            <ul class="space-y-3">
              <!-- ADDED POPULATION -->
              <li v-if="countryData2.population" class="flex justify-between items-center">
                <span class="font-semibold text-sm">Population ({{ countryData2.population.year }})</span>
                <span class="text-lime-300 font-mono text-lg">{{ formatNumber(countryData2.population.value) }}</span>
              </li>
